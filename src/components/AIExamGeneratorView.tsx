...
export const AIExamGeneratorView: React.FC<AIExamGeneratorViewProps> = ({
  ...
  const handleStartGeneration = async () => {
    ...
    setGenerationProgress('Đang tải cấu hình sư phạm và kiểm tra văn bản pháp lý...');

    try {
      setTimeout(() => {
        setGenerationProgress(`Đang xây dựng ma trận & bảng đặc tả môn ${currentProfileData.name} theo ${activeRegulation.document_number}...`);
      }, 500);

      setTimeout(() => {
        setGenerationProgress('Đang tạo hệ thống câu hỏi Phần I, II, III, IV với Gemini AI...');
      }, 1200);

      setTimeout(() => {
        setGenerationProgress('Đang chạy bộ kiểm định Validator (Đo lường 10,0đ, rubric tự luận, đáp án)...');
      }, 2000);
      ...
      {currentStep === 'structure' && (
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-6">
          ...
          <div className="flex justify-between pt-4 border-t border-slate-100">
            ...
            >
              <span>Tiếp tục: Ma trận & Bản đặc tả</span>
              <ArrowRight className="w-4 h-4 ml-2" />
            </button>
          </div>
        </div>
